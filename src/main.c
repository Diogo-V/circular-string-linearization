...
void traverse_tree(Node p, int n, char* T) {
    ...
    for (child = p->child; child != NULL; child = child->brother) {
...
void Descend(Point p, char c) {
    ...
    if (p->above != p->below) {
        if (string[p->below->head + p->depth] == c) {
            ...
            if (p->depth == p->below->depth) {  /* Is now in node */
                p->above = p->below;
            }

        }  /* Head of below node + depth of above node */
    }

    /* Finds branch to descend at */
    n = p->below->child;
    while (n != NULL) {
        if (string[n->head + p->above->depth] == c) {
            p->depth++;
            p->below = n;
...
void SuffixLink(Point p) {
    ...
    /* If only created leaf, then goes to parent's suffix link */
    if (p->above == p->below) {
        p->above = p->above->suffix_link;
        p->below = p->above;
        p->depth = p->above->depth;
        return;
    }

    /* Stores initial head to be used in the left branch */
    head = p->below->head;
    depth = p->depth - 1;
    ...
    /* string[head + p->depth + 1] == string[p->below->head + p->above->depth] */
    while (p->depth < depth) {
        p->above = p->below;
        Descend(p, string[head + p->depth + 1]);
        p->depth = p->below->depth;
    }
    p->depth = depth;

    /* Update last_internal_node to be the current node p->above */
    if (p->below == p->above && last != NULL) {
